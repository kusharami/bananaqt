...
namespace Banana
{
	...
	void ProjectDirectoryFilterModel::setSourceModel(QAbstractItemModel *sourceModel)
	{
		if (sourceModel != project_tree_model)
		{
			...
			{
				project_tree_model = dynamic_cast<ProjectDirectoryModel *>(sourceModel);
	...
	QVariant ProjectDirectoryFilterModel::data(const QModelIndex &index, int role) const
	{
				...
				if (source_model != nullptr)
				{
					...
					if (nullptr != project_dir)
					{
						if (ignore.exactMatch(project_dir->getRelativeFilePathFor(
												  source_model->fileInfo(mapToSource(index)).filePath())))
						{
	...
	void ProjectDirectoryFilterModel::setFileTypeFilter(const QMetaObject *meta_object)
	{
		beginResetModel();
	...
	void ProjectDirectoryFilterModel::applyFilters()
	{
		if (nullptr != project_file)
		{
			...
			{
				wildcards[i] = "(.*/)*" + wildcards[i].replace('\\', "\\\\").replace('.', "\\.").replace('*', ".*").replace('?', ".");
			}
	...
	bool ProjectDirectoryFilterModel::filterAcceptsColumn(int source_column, const QModelIndex &) const
	{
		return (0 == source_column);
	}

	bool ProjectDirectoryFilterModel::filterAcceptsRow(int source_row, const QModelIndex &source_parent) const
	{
		if (project_tree_model != nullptr)
		{
			...
			if (index.isValid())
			{
				...
				if (fileinfo.isDir()
				&&	!filePath.startsWith(QDir::cleanPath(project_dir->getFilePath()) + "/", Qt::CaseInsensitive))
					return true;
	...
	bool ProjectDirectoryFilterModel::lessThan(const QModelIndex &source_left, const QModelIndex &source_right) const
	{
		if (project_tree_model != nullptr)
		{
			...
			if (nullptr != project_dir)
			{
				if (!hideIgnored && ignore.isValid() && !ignore.isEmpty())
				{
					bool ignore_left = ignore.exactMatch(project_dir->getRelativeFilePathFor(left_info.filePath()));
					bool ignore_right = ignore.exactMatch(project_dir->getRelativeFilePathFor(right_info.filePath()));
			...
			int compare_names = QString::localeAwareCompare(left_info.baseName(), right_info.baseName());

			if (compare_names < 0)
				return true;

			int compare_suffixes = QString::localeAwareCompare(left_info.completeSuffix(), right_info.completeSuffix());
	...
	void ProjectDirectoryFilterModel::connectSourceModel()
	{
		if (nullptr != project_tree_model)
		{
			QObject::connect(project_tree_model, &QObject::destroyed,
							 this, &ProjectDirectoryFilterModel::onSourceModelDestroyed);
			QObject::connect(project_tree_model, &ProjectDirectoryModel::projectDirectoryChanged,
	...
	void ProjectDirectoryFilterModel::disconnectSourceModel()
	{
		if (nullptr != project_tree_model)
		{
			...
			QObject::disconnect(project_tree_model, &QObject::destroyed,
								this, &ProjectDirectoryFilterModel::onSourceModelDestroyed);
			QObject::disconnect(project_tree_model, &ProjectDirectoryModel::projectDirectoryChanged,
								this, &ProjectDirectoryFilterModel::applyProjectFile);
	...
	void ProjectDirectoryFilterModel::connectProjectFile()
	{
		if (nullptr != project_file)
		{
			QObject::connect(project_file, &QObject::destroyed,
							 this, &ProjectDirectoryFilterModel::onProjectFileDestroyed);
			QObject::connect(project_file, &AbstractProjectFile::changedHideIgnoredFiles,
							 this, &ProjectDirectoryFilterModel::applyFilters);
			QObject::connect(project_file, &AbstractProjectFile::changedIgnoredFilesPattern,
							 this, &ProjectDirectoryFilterModel::applyFilters);
		}
	}
	...
	void ProjectDirectoryFilterModel::disconnectProjectFile()
	{
		if (nullptr != project_file)
		{
			QObject::disconnect(project_file, &QObject::destroyed,
								this, &ProjectDirectoryFilterModel::onProjectFileDestroyed);
			QObject::disconnect(project_file, &AbstractProjectFile::changedHideIgnoredFiles,
								this, &ProjectDirectoryFilterModel::applyFilters);
			QObject::disconnect(project_file, &AbstractProjectFile::changedIgnoredFilesPattern,
								this, &ProjectDirectoryFilterModel::applyFilters);
