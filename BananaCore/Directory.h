...
namespace Banana
{
	...
	class Directory : public ObjectGroup, public AbstractDirectory, public FileNamingPolicy
	{
		...
		Q_PROPERTY(bool searched READ isSearched WRITE setSearched SCRIPTABLE true STORED false)
		Q_PROPERTY(int searchOrder READ getSearchOrder WRITE setSearchOrder SCRIPTABLE true STORED false)

	public:
		Q_INVOKABLE QString getAbsoluteFilePathFor(const QString &path, bool search = false) const;
		Q_INVOKABLE inline QString getRelativeFilePathFor(const QString &path) const;
		...
		static void registerFileType(const char *extension,
									 const QMetaObject *fileMetaObject,
									 const QMetaObject *dataMetaObject);
		static const QMetaObject *getFileTypeByExtension(const QString &filePath, const char **extensionPtr = nullptr);
		static const QMetaObject *getFileTypeByExtensionPtr(const char *extension);
		static std::vector<const char *> getFileTypeExtensions(const QMetaObject *metaObject);
		static const RegisteredFileType *findRegisteredFileType(const char *extension = nullptr,
																const QMetaObject *fileMetaObject = nullptr,
																const QMetaObject *dataMetaObject = nullptr);

		static QString getFileTypeTitle(const QMetaObject *metaObject, bool plural = false);
		static QString getFileFormatName(const char *extension, bool plural = false, bool emptyResult = false);
		static QString getFileFormatName(const QString &extension, bool plural = false, bool emptyResult = false);
		static QString getFileFormatNameFrom(const QString &filePath, bool plural = false);
		static QString getFilterForExtension(const char *extension);

		static QString getFileExtensionFromFilter(const QMetaObject *metaObject, const QString &filter);
		static QString getFilterForFileType(const QMetaObject *metaObject);
		static QStringList getPossibleFiltersForFileType(const QMetaObject *metaObject);
		...
		static void getDirContents(Directory *dir, std::set<AbstractFileSystemObject *> &output);
